...
import type { FavouriteProps } from './FavouriteToggle';
...
interface MediaCardProps<T extends FilmsWithGenres> {
    ...
    setMediaVisibleId: React.Dispatch<React.SetStateAction<number | null>>
...
function MediaCard<T extends FilmsWithGenres>({showGenres, lastMediaElementRef, setMediaVisibleId, media, type, favouriteIds, addFavourite, removeFavourite, isRef, addToHistory, setFavouriteIds} : MediaCardProps<T>) {
  ...
  return (
    <>
        <motion.div
            ...
            onClick={open}
            >
            <div onClick={() => { setMediaVisibleId(media.id)}} className="cursor-pointer">
            ...
            <motion.div className={`absolute inset-0 flex px-7 py-10  gap-2 ${media.adult ? `justify-between` : `justify-end`} items-center flex-col text-white bg-linear-to-b to-gray-800/80 from-gray-500/0 rounded-xl`}
                ...
                <div className="flex flex-row text-sm w-full gap-2 mb-2">
                    ...
                    { showGenres ? media.gatunki.map((g:string, i:number) => (
