...
function Switch({value, setValue, sliderColor = "bg-gray-400", sliderColorChange = "bg-white", backgroundColor = "bg-gray-200", backgroundColorChange = "bg-black"}: SwitchProps) {
  ...
  return (
    <>
        <motion.button layout className={`flex ${value ? `justify-end ${backgroundColorChange}` : `justify-start ${backgroundColor}`} w-12 h-6 cursor-pointer p-1 px-2 rounded-xl font-medium`} onClick={handleSwitch}>
